...
module.exports.logout = function (options) {
  ...
  function validateSamlResponse(req, sessionParticipant, cb) {
    var SAMLResponse = req.query.SAMLResponse || req.payload.SAMLResponse;
    ...
    if (req.payload.SAMLResponse || !options.deflate) {
  ...
  return function (req, h) {
    ...
    const res = {
      send: (t) => dt.send = t,
      set: (h,v) => dt.set.push({h,v}),
      sendStatus: (status) => dt.status = status,
      redirect: (x) => dt.redirect = x
    }
    return Promise.fromCallback(next => {
      try {
        // SP Initated flow.
        if (req.query.SAMLRequest || req.payload.SAMLRequest) {
          const opts = {
          ...
          parseIncomingLogoutRequest(req, req.query.SAMLRequest || req.payload.SAMLRequest, opts, function (err, requestData) {
            if (err) { return next(err); }
            ...
            options.sessionParticipants.get(requestData.issuer, requestData.sessionIndices, requestData.nameId, function (err, session) {
              ...
              const spData = {
                ...
                relayState: req.query.RelayState || (req.payload && req.payload.RelayState),
              };
        ...
        } else if (req.query.SAMLResponse || req.payload.SAMLResponse) {
          ...
          var h = req.query.RelayState || (req.payload && req.payload.RelayState);
      ...
      if(dt.send) {
        ret = ret.response(dt.send)
      } else {
        ret = ret.response('')
      }

      if(dt.redirect) {
        ret = ret.redirect(dt.redirect).temporary();
      }
      
      for(const { h,v  } of dt.set) {
        ret = ret.header(h,v);
      }
      
      if(dt.status) {
        ret = ret.code(dt.status)
      }
...
module.exports.sendLogoutError = function(options){
  return function(req, res, next){
    ...
    // Signature location
    options.reference = constants.ELEMENTS.LOGOUT_RESPONSE.SIGNATURE_LOCATION_PATH;
    options.relayState = options.relayState || req.query.RelayState || (req.payload || {}).RelayState;
